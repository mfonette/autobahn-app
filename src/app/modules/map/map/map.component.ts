...
export class MapComponent implements OnInit, AfterViewInit {
  ...
  private addParkingLorryToMap(highway:any): void {
    this.subscription.add(this.autobahnService.fetchServiceDetails(highway, 'parking_lorry').subscribe(data => {
      this.addItemsToMap(data.parking_lorry);
    }));
  }

  private addElectricChargingStationsToMap(highway:any): void {
    this.subscription.add(this.autobahnService.fetchServiceDetails(highway, 'electric_charging_station').subscribe(data => {
      this.addItemsToMap(data.electric_charging_station);
    }));
  }

  private addClosuresToMap(highway:any): void {
    this.subscription.add(this.autobahnService.fetchServiceDetails(highway, 'closure').subscribe(data => {
